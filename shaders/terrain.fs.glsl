...
vec3 getTriPlanarTexture(int textureId) {
    ...
    vec3 xaxis = texture(uTexture, vec3(vert_position.yz * 0.5, textureId)).rgb;
    vec3 yaxis = texture(uTexture, vec3(vert_position.xz * 0.5, textureId)).rgb;
    vec3 zaxis = texture(uTexture, vec3(vert_position.xy * 0.5, textureId)).rgb;
...
vec3 finalTexture(int index) {
    float n1 = (texture(uTextureNoise, vert_texCoord / 0.3).r * 0.05) + 0.95;
    float n2 = (texture(uTextureNoise, vert_texCoord / 4.5).r * 0.1) + 0.90;
...
float circle() {
    ...
    float border = 0.08;
    float dist = distance(uMousePosition.xz, vert_position.xz);
...
void main() {
    ...
    for (int i = 0; i < uSplatCount; i++) {
        float intesity = texture(uSplatMap, vec3(vert_texCoord.x, vert_texCoord.y, i)).r;
        if (intesity > 0.0) {
    ...
    color *= getShadowFactor(vert_shadowCoords, 0.3);
